...
2. Applies the Zscaler Cloud Connector Terraform plan
================
"
...
# Here we update terraform.tfvars. We could just create a new file, but then we would lose access to the other settings too
# Instead of replacing existing enties we just comment them out. This also removes any Linux/MACOS 'sed' conflicts)
...
#now add the new values at the top (in reverse order)
printf '%s\n' 0a $'cc_vm_managed_identity_resource_group       = "'$CC_RG_NEW'"' . x | ex -s terraform.tfvars
printf '%s\n' 0a $'cc_vm_managed_identity_name                 = "'$MANAGED_ID_NEW'"' . x | ex -s terraform.tfvars
printf '%s\n' 0a $'azure_vault_url                             = "'$VAULT_URL_NEW'"' . x | ex -s terraform.tfvars
printf '%s\n' 0a $'http_probe_port                             = "'$PROBE_PORT_NEW'"' . x | ex -s terraform.tfvars
printf '%s\n' 0a $'cc_vm_prov_url                              = "'$PROV_URL_NEW'"' . x | ex -s terraform.tfvars

# and export the local Azure values
...
read -p "${purple}Everything is ready and stored locally. You can continue, or run \"zsec up\" in the future. Continue? [y|n]: ${reset}" deploy
echo ""
if [ $deploy = "y" ]
then
    # Ask User for inputs on naming prefix for  objects, Azure region
    ...
    echo "${red}Cancelling. Run \"zsec up\" to deploy Cloud Connectors using the parameters defined ${reset}"
    exit
fi
